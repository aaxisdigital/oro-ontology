...
class OntologyDataViewComponent extends BaseComponent {
    constructor() {
        ...
        this.$addEntity = null;
        this.$addFormat = null;
        this.$addPayload = null;
        this.$addPayloadError = null;
        this.$addSubmit = null;
    }
    initialize(options) {
        ...
        this.grid = new DataGrid({
            columns: [
                ...
                { key: 'version', label: __('aaxis.ontology.data_view.version'), type: 'number', width: '110px', hidden: true },
                {
                    ...
                    width: '190px', render: (row) => this.renderDate(row.updatedAt)
                },
                {
                    key: 'payload', label: __('aaxis.ontology.data_view.payload'), type: 'json',
                    sortable: false, copyValue: (row) => String(row.payload || ''),
                    render: (row) => this.renderPayload(row)
                }
            ],
            actions: [
        ...
        this.$el.on('click.aaxisOntologyData', '[data-role="refresh"]', (e) => {
            e.preventDefault();
            this.load();
        });
        this.$el.on('click.aaxisOntologyData', '[data-role="columns-settings"]', (e) => {
            e.preventDefault();
            this.grid.toggleColumnSettings(e.currentTarget);
        });
        this.$el.on('click.aaxisOntologyData', '[data-role="add"]', (e) => {
            e.preventDefault();
            this.openAddData();
        });
        this.load();
     ...
     */
    renderVersions($content, versions) {
        ...
        const $selectField = $('<div/>', { 'class': 'aaxis-json-version' }).append($('<label/>', { 'class': 'aaxis-json-version__label', text: __('aaxis.ontology.data_view.version') }), $select);
        ...
        const $copy = $('<button/>', { type: 'button', 'class': 'btn btn-sm aaxis-json-copy' }).append($('<span/>', { 'class': 'fa fa-clipboard', 'aria-hidden': 'true' }), $('<span/>', { text: ' ' + __('aaxis.ontology.data_view.copy') }));
     ...
     */
    renderVersionDiffHtml(selected, previous) {
     ...
     */
    renderValueDiff(sel, prev, indent) {
    ...
    renderObjectDiff(sel, prev, indent) {
     ...
     */
    renderArrayDiff(sel, prev, indent) {
        const pad = '  '.repeat(indent);
        const padIn = '  '.repeat(indent + 1);
    ...
    openAddData() {
        ...
        const $format = $('<select/>', { 'class': 'form-control', 'data-role': 'format' }).append($('<option/>', { value: 'json', text: 'JSON' }), $('<option/>', { value: 'csv', text: 'CSV' }));
        ...
        $payloadField.append($('<label/>', { 'class': 'aaxis-rfm__label', text: __('aaxis.ontology.data_view.payload') }), $payload, $payloadError);
    ...
    rebuildEntityOptions($entity, systemId) {
    ...
    setAddSubmitBusy(busy) {
        if (busy) {
            this.$addSubmit.prop('disabled', true).empty().append($('<span/>', { 'class': 'fa fa-spinner fa-spin', 'aria-hidden': 'true' }), $('<span/>', { text: ' ' + __('aaxis.ontology.data_view.sending') }));
            return;
        }
     ...
     */
    highlightSearch($pre, syntaxHtml, query) {
        $pre.html(syntaxHtml);
