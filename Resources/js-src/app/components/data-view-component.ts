...
class OntologyDataViewComponent extends BaseComponent {
    ...
    // The JSON render/diff machinery lives in the shared widgets/json-diff module (the flow
    // editor's View source / Flow history dialogs use the same functions); these thin wrappers
    // keep this component's many call sites unchanged.
    private renderVersionDiffHtml(selected: any, previous: any): string {
        return renderVersionDiffHtml(selected, previous);
    }

    private pruneToDiff(sel: any, prev: any): {sel: any; prev: any} | null {
        return pruneToDiff(sel, prev);
    }
