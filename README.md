...
| Connectors | `aaxis_ontology_connectors` | Oro datagrid + CRUD | Integrations (SFTP / REST API / File System, plus Database and Bucket whose configuration forms are still to be defined) for a system; the per-type JSON config is read-only on the form and edited through a type-specific "Configure…" popup, with secret values (passwords, keys, Authorization headers) always shown masked |
| Flows | `aaxis_ontology_flows` | TS DataGrid | Named, toggleable pipelines whose ordered steps are stored as JSON, with their Last executed / Last modified timestamps. Each row can be **exported** to a JSON file, and an **Import Flow** button (before "Add Flow") recreates one from such a file in another environment — see *Moving flows between environments* below |
| Flow editor | `aaxis_ontology_flow_editor` | TS page | Opened by "Add Flow" / a row's edit action: flow name + enabled switch (left), Toolbox show/hide toggle + Close/Cancel + Save (right — Save enables only with pending changes, the exit button reads Close when the state is saved, and saving stays on the page) over a dot-matrix canvas with a draggable, hideable step toolbox (Triggers / Actions / Operations). Steps are dragged onto the canvas as square tiles (icon + unique step name below it — up to two rows — defaulting to `type-n`), movable and always grid-snapped; double-click opens the step's settings panel (modal, next to the tile) where the name — and the step's configuration (Schedule: an every-N interval (minute/hour/day/week/month/year) or a linux cron expression, whose textbox tints light red while invalid and shows symbol guidance for the cron field being edited · Entity change: system + entity · Reader: entity-sourced — all records (never page-capped for flows; optionally ordered by any entity attribute asc/desc and/or limited to 1/10/100/1000) or one by id — or connector-sourced with path, plus operation and body (+ content) for REST API connectors; destination defaults to `payload` · DWL transform: a DataWeave script (bundled engine — including date/time literals like `|2003-10-01|` / `|PT1S|`, date ± period arithmetic and `{format: …}` date coercions) that sees every debug-context value as a variable and writes its result to the destination · Writer: like the reader, but the entity variant takes a Content — the context value to write (single object or array, unique ids may not repeat within one write), written synchronously through the same validation and upsert function as the Data View "Add Data", stamped with the flow being debugged (Manual only when the flow was never saved); its receipt reports the real outcome — `{uuid, count, upsert, changedIds}` — and the flow's event row completes immediately; an empty content (null / `[]` / `""`) is not an error — the write is skipped and the receipt reports zeros; a writer on a file/SFTP/bucket connector also takes a Content field — DWL-capable, in its own row under the path — since those connectors have no request body to carry what to write). The connector Body content and the writer Content carry a "DWL" on/off switch next to their label: on, the field is evaluated as a DWL expression against the execution context (the body sends string results verbatim and JSON-encodes the rest; the writer's result is the record(s) to write) — off keeps the literal text / context-key behavior; DWL-toggled fields are parse-validated on save. Step settings that load catalogs (systems, entities, connectors) show a blocking spinner over the properties popup until the data is in — is edited; a flow can hold only one trigger (dropping a second asks to replace). Tiles are wired by dragging from the "×" output port on their right edge (Choice has two ports) to another tile — arrows arrive at the target's left side; one incoming link per element, triggers accept none. Lines route orthogonally around tiles (never over an element, deviating shortly before it) and draw a small round "jump" where one line crosses another. Steps not reachable from the trigger through the flow lines are shown with a light gray background — they would not execute. In sub-flows (no trigger), right-click an element with no incoming line and choose "Start here" to mark the entry point (an origin-less arrow); adding a trigger later asks before replacing that marker. Tiles are selectable (click or macOS-style rubber band) and a multi-selection drags as a group; right-click opens a context menu with Remove, and for multi-selections Align (one row starting at the leftmost tile with one-tile gaps — steps already chained by flow lines keep their flow sequence, then any unconnected ones follow by X position) and Connect (chains the selection left-to-right when the wiring rules allow); right-clicking a flow line offers Remove for that single connection. The canvas fills the page and scrolls whenever a step sits beyond the visible area; an **Organize** button lays all steps out in execution order, wrapping rows at the visible width. **Run Now** and **Debug** (flows with a trigger; both disabled while the flow's Enabled switch is off) open a debugger sidebar left of the design area — cron runs immediately, entity change asks for system/entity + a JSON payload first. Debug steps through the execution: after every step the sidebar lists the variables one by one (objects/arrays start collapsed with their element count and expand on click), offers the Next step / Run all / Cancel buttons, and a DWL evaluator — just above the buttons — that runs any expression against the current variables at any point, showing the result in a modal; the tile that just executed is highlighted in amber on the canvas. Run Now executes everything in one go and the same sidebar appears once the run finishes, showing only the final result (all steps of a session share one `flowUuid`); Escape closes any step properties popup like its Cancel button. Every top-level execution mints one uuid, visible in the output as `flowUuid` (a reserved destination name, directly usable in DWL scripts): all writes of the run are stamped with it, grouping their events and records under a single identity even when the flow holds several writers — sub-flows inherit the caller's uuid instead of minting their own. Fields that accept pure text or DWL share one component (title, the switch always right-aligned over the textarea — hidden where the content must be DWL, like the transform's code — and the textarea) that pretty-prints the code whenever DWL mode is active; the Entities DWL playground's script editor reuses the same component. Entity readers return `null` for a missing "by id" record; REST API connector readers perform the real HTTP call (including OAuth token fetch when configured). The canvas state is saved in the flow's `design` and restored on edit (a corrupted/outdated design is reported and the editor starts empty). Dot spacing and tile size (default 8× the spacing) are configurable in System Configuration → Aaxis Ontology → Flows |
| Events | `aaxis_ontology_events` | TS DataGrid | Read-only log of flow executions (which unique ids were seen / changed). Both id columns lead with how many there are — `(3) - a, b, c` — and every UUID carries a funnel button that filters the grid down to the events sharing it (clear it from the column's own filter popover); copying a cell still yields the raw ids |
| Data View | `aaxis_ontology_data_view` | TS DataGrid | Consolidated, versioned data records that flowed through the model. Each row carries an **Update** action (before Versions) that reopens the "Add Data" form preloaded with that record — system and entity locked, the payload pretty-printed — and only enables Submit once the payload is actually edited; saving goes through the same upsert, so the record gains a new version |
...
2. **`OntologyDataUpsertProcessor`** opens an event row in `aaxis_ontology_data_events`, then
   delegates validation + upsert to the PostgreSQL function **`aaxis_ontology_data_upsert(jsonb)`**
   and closes that row with the ids it actually created/updated and the finish time — whether the
   batch succeeded, was rejected or crashed. Both this consumer and the synchronous path used by
   flow writer steps go through **`Manager/OntologyDataEventRecorder`**, the single writer of the
   events table, so the Events page reports the same way for every kind of run.
...
A flow row's **Export** action downloads it as a self-contained JSON file, and the **Import Flow**
button on the same page recreates it elsewhere.

Flows are almost portable already — their steps name systems and entities rather than pointing at
database ids — with one exception: connector steps hold the connector's numeric id, which means
nothing in another database. The export therefore replaces every connector id with a
`{name, type, system}` descriptor, and the import resolves those back to local ids. It also records
each referenced entity's **unique attribute**, which the steps themselves do not carry, so the
target environment can be checked for a same-named entity that is keyed differently.

Before anything is created, the import checks — and reports **every** problem at once:

- the file is a flow export of a supported format and canvas version;
- the flow name is not already taken here;
- every referenced connector exists here with the same name, type and system (an ambiguous or
  incompletely described connector is refused rather than guessed);
- every referenced entity exists here under the same system name and with the same unique attribute;
- the steps clear exactly the same rules the flow editor enforces when saving.

Imported flows are always created **disabled**, so nothing starts running on the strength of a file.

---

## Scheduled flow execution

Flows whose trigger is a **Schedule** run automatically: the `aaxis:ontology:flows:run-due`
command is registered with Oro's cron to run **every minute** (`oro:cron:definitions:load` after
deploying) and executes every enabled flow that is due:

- **Interval** schedules run when `value × unit` has elapsed since the flow's last execution —
  or since its creation when it never ran.
- **Cron** schedules run when the current minute matches the cron expression.

Due flows execute through the same engine as the editor's "Run Now" (same `flowUuid`,
`last_executed` stamping, synchronous writes and event rows); a failing flow is logged and does
not block the others. The command can also be run by hand for a one-off sweep. Candidate lookup
uses the denormalized `trigger_type` column, kept up to date on every save.
